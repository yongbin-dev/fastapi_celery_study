...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "started_at", sa.DateTime(timezone=True), nullable=True, comment="시작 시간"
        ),
        sa.Column(
            "finished_at", sa.DateTime(timezone=True), nullable=True, comment="완료 시간"
        ),
        sa.Column(
            "initiated_by", sa.String(length=100), nullable=True, comment="시작한 사용자/시스템"
        ),
        sa.Column(
            "input_data",
    ...
    op.create_index(
        "idx_chain_name_status",
        "chain_executions",
        ["chain_name", "status"],
        unique=False,
    )
    op.create_index(
        "idx_chain_status_created",
        "chain_executions",
        ["status", "created_at"],
        unique=False,
    )
    op.create_index(
        op.f("ix_chain_executions_chain_id"),
        "chain_executions",
        ["chain_id"],
        unique=True,
    )
    op.create_index(
        op.f("ix_chain_executions_chain_name"),
        "chain_executions",
        ["chain_name"],
        unique=False,
    )
    op.create_index(
        op.f("ix_chain_executions_status"), "chain_executions", ["status"], unique=False
    )
    op.create_table(
        "users",
