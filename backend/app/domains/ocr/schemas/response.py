...
class TextBoxDTO(BaseModel):
    ...
    bbox: List[List[float]] = Field(..., description="바운딩 박스 좌표 [[x1,y1], [x2,y2], [x3,y3], [x4,y4]]")


class OCRResultDTO(BaseModel):
    """OCR 실행 결과 DTO"""

    text_boxes: List[TextBoxDTO] = Field(default_factory=list, description="추출된 텍스트 박스 목록")
