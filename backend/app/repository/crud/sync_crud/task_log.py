...
#         """재시도 횟수 증가"""
#         task_log.retries = (task_log.retries or 0) + 1  # type: ignore
#         db.add(task_log)
#         db.commit()
#         db.refresh(task_log)
#         return task_log

#     def get_by_chain_tasks(self, db: Session, *, chain_id: str) -> List[TaskLog]:
#         """특정 체인의 모든 작업 로그 조회 (순서대로)"""
#         # pipeline 작업들만 필터링 (app.tasks.stage로 시작하는 것들)
#         return (
#             db.query(TaskLog)
#             .filter(TaskLog.task_name.like("app.tasks.stage%"))
#             .order_by(TaskLog.task_name)
#             .all()
#         )

#     def get_pipeline_tasks_by_chain(
#         self, db: Session, *, chain_id: str
#     ) -> List[TaskLog]:
#         """파이프라인 작업들을 단계순으로 조회"""
#         stage_tasks = []
...
#             tasks = (
#                 db.query(TaskLog)
#                 .filter(TaskLog.task_name.like(f"app.tasks.stage{stage_num}_%"))
#                 .order_by(desc(TaskLog.created_at))
#                 .first()
#             )
#             if tasks:
...
#     ) -> List[TaskLog]:
#         """최근 N일간 작업 로그 목록 조회"""
#         # from datetime import timedelta  # 이미 상단에서 import됨

#         since_date = datetime.now() - timedelta(days=days)
#         return (
#             db.query(TaskLog)
#             .filter(TaskLog.created_at >= since_date)
#             .order_by(desc(TaskLog.created_at))
#             .limit(limit)
#             .all()
#         )
...
#             count = db.query(TaskLog).filter(TaskLog.status == status).count()
#             stats[status] = count
#         return stats

#     def get_task_performance_stats(self, db: Session, *, task_name: str) -> dict:
#         """특정 작업의 성능 통계"""
#         # from sqlalchemy import func  # 아래에서 직접 import
...
#             "avg_duration": float(getattr(result, 'avg_duration', 0) or 0),
#             "min_duration": float(getattr(result, 'min_duration', 0) or 0),
#             "max_duration": float(getattr(result, 'max_duration', 0) or 0),
#         }
