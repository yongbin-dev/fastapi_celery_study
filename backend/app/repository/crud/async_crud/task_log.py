...
class AsyncCRUDTaskLog(AsyncCRUDBase[TaskLog, TaskLogCreate, TaskLogUpdate]):  # type: ignore
...
#             stmt = (
#                 select(TaskLog)
#                 .where(TaskLog.status.in_(["SUCCESS", "FAILURE", "REVOKED"]))  # type: ignore
...
#         self, db: AsyncSession, *, task_log: TaskLog
#     ) -> TaskLog:
#         """재시도 횟수 증가"""
#         try:
...
#             stmt = (
#                 select(TaskLog)
#                 .where(TaskLog.created_at >= since_date)
#                 .order_by(desc(TaskLog.created_at))
#                 .limit(limit)
#             )
#             result = await db.execute(stmt)
...
#         self, db: AsyncSession, *, task_name: str
#     ) -> dict:
#         """특정 작업의 성능 통계"""
#         try:
#             # 성공한 작업들의 실행 시간 통계
...
#
#             return {
#                 "total_count": getattr(row, 'total_count', 0) or 0,
#                 "avg_duration": float(getattr(row, 'avg_duration', 0) or 0),
#                 "min_duration": float(getattr(row, 'min_duration', 0) or 0),
#                 "max_duration": float(getattr(row, 'max_duration', 0) or 0),
#             }
#         except Exception as e:
#             await db.rollback()
#             raise e
