...
from app.models import TaskLog, QueueStats
    ...
    with get_db_manager().get_sync_session() as session:
        ...
        try:
            ...
            if body:
                ...
                task_args = (
                    body[1] if isinstance(body, (list, tuple)) and len(body) > 1 else []
                )
                task_kwargs = (
                    body[2] if isinstance(body, (list, tuple)) and len(body) > 2 else {}
                )

                # Pipeline task인지 확인
                if task_name and "stage" in task_name:
                    chain_id = get_chain_id_from_args(
                        task_args
                    ) or get_chain_id_from_kwargs(task_kwargs)
                    if chain_id:
                        logger.info(
                            f"Pipeline task 발행: {task_name}, chain_id: {chain_id}"
                        )

                        # ChainExecution이 존재하지 않으면 생성
                        existing_chain = chain_execution_crud.get_by_chain_id(
        ...
        try:
            ...
            if not existing_task:
                ...
                task_log_crud.update_status(
                    session, task_log=existing_task, status="STARTED"
                )
            ...
            task_metadata_crud.create_task_metadata(
                ...
                priority=getattr(task.request, "priority", 0)
                if hasattr(task, "request")
                else 0,
            )
            ...
            attempt_number = (
                task_execution_history_crud.get_retry_count(session, task_id=task_id)
                + 1
            )
            ...
            logger.info(
                f"TaskExecutionHistory 생성: {task_id}, attempt: {attempt_number}"
            )
    ...
    with get_db_manager().get_sync_session() as session:
        ...
        try:
            ...
            if task_log:
                ...
                latest_attempt = task_execution_history_crud.get_latest_attempt(
                    session, task_id=task_id
                )
                if latest_attempt:
                    task_execution_history_crud.complete_attempt(
                        session, execution_history=latest_attempt, status="SUCCESS"
                    )
                ...
                if task_log.task_name and "stage" in task_log.task_name:
                    ...
                    if chain_id:
                        chain_exec = chain_execution_crud.get_by_chain_id(
                            session, chain_id=chain_id
                        )
                        if chain_exec:
...
def task_failure_handler(
    ...
    with get_db_manager().get_sync_session() as session:
        ...
        try:
            ...
            if task_log:
                task_log_crud.update_status(
                    session, task_log=task_log, status="FAILURE", error=error_message
                )
    ...
    with get_db_manager().get_sync_session() as session:
        ...
        try:
            ...
            if task_log:
                ...
                task_log_crud.update_status(
                    session, task_log=task_log, status="RETRY", error=error_message
                )
            ...
            task_execution_history_crud.create_attempt(
                session, task_id=task_id, attempt_number=attempt_number, status="RETRY"
            )
            logger.info(
                f"TaskExecutionHistory 재시도 기록: {task_id}, attempt: {attempt_number}"
            )
...
def collect_queue_stats():
    ...
    with get_db_manager().get_sync_session() as session:
        ...
        for worker_name, tasks in (active_tasks or {}).items():
            # QueueStats 업데이트
            queue_stat = (
                session.query(QueueStats)
                .filter_by(queue_name="celery", worker_name=worker_name)
                .first()
            )
...
def get_task_statistics(session):
    ...
    try:
        stats = (
            session.query(
                TaskLog.status,
                session.query(TaskLog).filter_by(status=TaskLog.status).count(),
            )
