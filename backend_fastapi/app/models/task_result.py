...
class TaskResult(Base):
    ...
    result_type: Column[str] = Column(
        String(50), default="json", comment="결과 타입 (json/binary/text/pickle)"
    )
    result_data = Column(Text, comment="텍스트 형식 결과")
    result_binary = Column(LargeBinary, comment="바이너리 형식 결과")
    result_size: Column[Integer] = Column(Integer, comment="결과 크기 (바이트)")
    ...
    def to_dict(self, include_data=False):
        ...
        if include_data:
            if self.result_type in ["json", "text"]:
                ...
                # Base64 인코딩하여 전송
                result["data"] = (
                    base64.b64encode(self.result_binary).decode("utf-8")
                    if self.result_binary
                    else None
                )
            elif self.result_type == "pickle":
