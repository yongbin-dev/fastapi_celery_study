...
class TaskMetadata(Base):
    ...
    # 워커 정보
    worker_name = Column(String(255), index=True, comment="워커 이름 (예: celery@worker-01)")

    # 큐 및 라우팅 정보
    queue_name = Column(
        String(255), default="default", comment="큐 이름 (예: default, priority)"
    )
    exchange = Column(String(255), comment="Exchange 이름 (AMQP)")
    routing_key = Column(String(255), comment="라우팅 키 (AMQP)")

    # 작업 스케줄링
    priority = Column(Integer, default=0, comment="작업 우선순위 (0-9, 높을수록 우선)")
    eta = Column(DateTime, comment="예정 실행 시간 (ETA: Estimated Time of Arrival)")
    expires = Column(DateTime, comment="작업 만료 시간")

    # 작업 체인/그룹 정보
    parent_id = Column(String(255), comment="부모 작업 ID (체인/그룹)")
    root_id = Column(String(255), comment="루트 작업 ID (워크플로우)")
