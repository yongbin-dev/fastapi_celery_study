...
class TaskDependency(Base):
    ...
    # 조건부 의존성
    condition = Column(String(255), comment="조건부 의존성 조건 (예: success_only, any_status)")
    ...
    # 관계 설정
    task = relationship(
        "TaskLog", foreign_keys=[task_id], back_populates="dependencies"
    )

    depends_on = relationship(
        "TaskLog", foreign_keys=[depends_on_task_id], back_populates="dependents"
    )
    ...
    def get_dependency_graph(cls, session, root_task_id, depth=None):
        ...
        return {root_task_id: {"children": build_graph(root_task_id)}}
