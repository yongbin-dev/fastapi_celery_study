...
class QueueStats(Base):
    ...
    # 메타데이터
    measured_at = Column(DateTime, default=datetime.now, index=True, comment="측정 시간")
    ...
    def get_latest_stats(cls, session, queue_name, hours=1):
        ...
        return (
            session.query(cls)
            .filter(cls.queue_name == queue_name, cls.measured_at >= since)
    ...
    def aggregate_stats(cls, stats_list):
        ...
        return {
            ...
            "avg_execution_time": sum(s.avg_execution_time or 0 for s in stats_list)
            / len(stats_list),
            "avg_throughput": sum(s.throughput or 0 for s in stats_list)
            / len(stats_list),
            "avg_error_rate": sum(s.error_rate or 0 for s in stats_list)
            / len(stats_list),
