...
class CRUDTaskMetadata(CRUDBase[TaskMetadata, dict, dict]):
    ...
    def get_by_task_id(self, db: Session, *, task_id: str) -> Optional[TaskMetadata]:
        ...
        return db.query(TaskMetadata).filter(TaskMetadata.task_id == task_id).first()

    def get_by_worker(
        self, db: Session, *, worker_name: str, skip: int = 0, limit: int = 100
    ) -> List[TaskMetadata]:
        """워커별 작업 메타데이터 목록 조회"""
        return (
            db.query(TaskMetadata)
            .filter(TaskMetadata.worker_name == worker_name)
            .order_by(desc(TaskMetadata.created_at))
            .offset(skip)
            .limit(limit)
            .all()
        )
    ...
    def get_by_queue(
        self, db: Session, *, queue_name: str, skip: int = 0, limit: int = 100
    ) -> List[TaskMetadata]:
        """큐별 작업 메타데이터 목록 조회"""
        return (
            db.query(TaskMetadata)
            .filter(TaskMetadata.queue_name == queue_name)
            .order_by(desc(TaskMetadata.created_at))
            .offset(skip)
            .limit(limit)
            .all()
    ...
    def get_by_priority(
        ...
        *,
        ...
        return (
            ...
            .order_by(desc(TaskMetadata.priority), desc(TaskMetadata.created_at))
            .offset(skip)
            .limit(limit)
            .all()
        )
    ...
    def get_scheduled_tasks(
        ...
        return (
            db.query(TaskMetadata)
            .filter(
                and_(TaskMetadata.eta.isnot(None), TaskMetadata.eta > datetime.now())
            )
            .order_by(TaskMetadata.eta)
            .offset(skip)
            .limit(limit)
            .all()
        )
    ...
    def get_expired_tasks(
        ...
        return (
            ...
            .order_by(desc(TaskMetadata.expires))
            .offset(skip)
            .limit(limit)
            .all()
        )
    ...
    def get_by_parent_id(
        self, db: Session, *, parent_id: str, skip: int = 0, limit: int = 100
    ) -> List[TaskMetadata]:
        """부모 작업 ID로 자식 작업들 조회"""
        return (
            db.query(TaskMetadata)
            .filter(TaskMetadata.parent_id == parent_id)
            .order_by(desc(TaskMetadata.created_at))
            .offset(skip)
            .limit(limit)
            .all()
    ...
    def get_by_root_id(
        self, db: Session, *, root_id: str, skip: int = 0, limit: int = 100
    ) -> List[TaskMetadata]:
        """루트 작업 ID로 워크플로우 작업들 조회"""
        return (
            db.query(TaskMetadata)
            .filter(TaskMetadata.root_id == root_id)
            .order_by(desc(TaskMetadata.created_at))
            .offset(skip)
            .limit(limit)
            .all()
    ...
    def get_with_task(self, db: Session, *, task_id: str) -> Optional[TaskMetadata]:
        """작업 로그와 함께 메타데이터 조회"""
        return (
            db.query(TaskMetadata)
            .options(joinedload(TaskMetadata.task))
    ...
    def get_worker_statistics(self, db: Session) -> List[dict]:
        ...
        result = (
            ...
            .filter(TaskMetadata.worker_name.isnot(None))
            .group_by(TaskMetadata.worker_name)
    ...
    def get_queue_statistics(self, db: Session) -> List[dict]:
        ...
        result = (
            db.query(
                ...
                func.count(TaskMetadata.id).label("task_count"),
                func.avg(TaskMetadata.priority).label("avg_priority"),
                func.count(func.distinct(TaskMetadata.worker_name)).label(
                    "worker_count"
                ),
            )
            .group_by(TaskMetadata.queue_name)
            .order_by(desc("task_count"))
            .all()
        )
    ...
    def get_priority_distribution(self, db: Session) -> dict:
        ...
        result = (
            db.query(TaskMetadata.priority, func.count(TaskMetadata.id).label("count"))
            .group_by(TaskMetadata.priority)
            .order_by(TaskMetadata.priority)
            .all()
        )
    ...
    def get_workflow_tree(self, db: Session, *, root_id: str) -> dict:
        """워크플로우 트리 구조 조회"""
        # 루트 작업 조회
        root_metadata = (
            db.query(TaskMetadata)
            .filter(
                and_(TaskMetadata.root_id == root_id, TaskMetadata.parent_id.is_(None))
            )
            .first()
        )
        ...
        # 모든 워크플로우 작업들 조회
        all_tasks = db.query(TaskMetadata).filter(TaskMetadata.root_id == root_id).all()
    ...
    def get_recent_tasks_by_worker(
        self, db: Session, *, worker_name: str, days: int = 7, limit: int = 100
    ) -> List[TaskMetadata]:
