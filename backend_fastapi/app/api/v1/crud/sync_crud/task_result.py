...
class CRUDTaskResult(CRUDBase[TaskResult, dict, dict]):
    ...
    def get_by_task_id(self, db: Session, *, task_id: str) -> Optional[TaskResult]:
        """task_id로 작업 결과 조회"""
        return db.query(TaskResult).filter(TaskResult.task_id == task_id).first()

    def get_by_result_type(
        self, db: Session, *, result_type: str, skip: int = 0, limit: int = 100
    ) -> List[TaskResult]:
        """결과 타입별 작업 결과 목록 조회"""
        return (
            db.query(TaskResult)
            .filter(TaskResult.result_type == result_type)
    ...
    def get_large_results(
        ...
        *,
        ...
        return (
            db.query(TaskResult)
            .filter(TaskResult.result_size >= min_size)
    ...
    def get_with_task(self, db: Session, *, task_id: str) -> Optional[TaskResult]:
        """작업 로그와 함께 결과 조회"""
        return (
            db.query(TaskResult)
    ...
    def create_task_result(
        self, db: Session, *, task_id: str, data: Any, result_type: str = "auto"
    ) -> TaskResult:
        """새 작업 결과 생성"""
    ...
    def get_result_data(self, db: Session, *, task_id: str) -> Optional[Any]:
        """작업 결과 데이터 조회"""
    ...
    def get_result_preview(
        self, db: Session, *, task_id: str, max_length: int = 100
    ) -> Optional[str]:
        """작업 결과 미리보기 조회"""
        task_result = self.get_by_task_id(db, task_id=task_id)
        if task_result:
    ...
    def cleanup_old_results(self, db: Session, *, days: int = 90) -> int:
        """오래된 결과 정리"""

        cleanup_date = datetime.now() - timedelta(days=days)
        deleted_count = (
            db.query(TaskResult).filter(TaskResult.created_at < cleanup_date).delete()
        )
        db.commit()
        return deleted_count

    def cleanup_large_results(
        self, db: Session, *, max_size: int = 100 * 1024 * 1024, days: int = 30  # 100MB
    ) -> int:
        """대용량 오래된 결과 정리"""
