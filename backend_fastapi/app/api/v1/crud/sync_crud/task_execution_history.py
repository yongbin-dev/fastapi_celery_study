...
class CRUDTaskExecutionHistory(CRUDBase[TaskExecutionHistory, dict, dict]):
    ...
    def get_by_task_id(
        self, db: Session, *, task_id: str, skip: int = 0, limit: int = 100
    ...
    def get_by_task_id_and_attempt(
        self, db: Session, *, task_id: str, attempt_number: int
    ...
    def get_by_status(
        ...
        return (
            db.query(TaskExecutionHistory)
            .filter(TaskExecutionHistory.status == status)
    ...
    def get_failed_attempts(
        ...
        return (
            db.query(TaskExecutionHistory)
            .filter(TaskExecutionHistory.status.in_(["FAILURE", "TIMEOUT"]))
            .order_by(desc(TaskExecutionHistory.created_at))
            .offset(skip)
            .limit(limit)
            .all()
    ...
    def get_with_task(self, db: Session, *, task_id: str) -> List[TaskExecutionHistory]:
        """작업 로그와 함께 실행 이력 조회"""
        return (
            db.query(TaskExecutionHistory)
    ...
    def create_attempt(
        self, db: Session, *, task_id: str, attempt_number: int, status: str = "STARTED"
    ...
    def get_retry_count(self, db: Session, *, task_id: str) -> int:
        """특정 작업의 재시도 횟수 조회"""
        return (
            db.query(TaskExecutionHistory)
            .filter(TaskExecutionHistory.task_id == task_id)
    ...
    def get_failure_patterns(self, db: Session, *, limit: int = 100) -> List[dict]:
        """실패 패턴 분석"""
        ...
        return [
            {"error_pattern": row.error_pattern, "count": row.count} for row in result
        ]

    def get_execution_stats(
        self, db: Session, *, task_id: Optional[str] = None
    ) -> dict:
        """실행 통계 조회"""
        from sqlalchemy import func
        ...
        return {
            ...
            "success_rate": round(
                (success_count / total_attempts * 100) if total_attempts > 0 else 0, 2
            ),
            "failure_rate": round(
                (failure_count / total_attempts * 100) if total_attempts > 0 else 0, 2
            ),
            "avg_duration": float(result.avg_duration or 0),
        }

    def get_recent_failures(
        self, db: Session, *, days: int = 7, limit: int = 50
    ) -> List[TaskExecutionHistory]:
        """최근 N일간 실패한 시도들 조회"""
        from datetime import datetime, timedelta
    ...
    def get_long_running_attempts(
        self, db: Session, *, min_duration_seconds: int = 300, limit: int = 50  # 5분
    ) -> List[TaskExecutionHistory]:
        """장시간 실행된 시도들 조회"""
        return (
            db.query(TaskExecutionHistory)
    ...
    def cleanup_old_history(self, db: Session, *, days: int = 180) -> int:
        """오래된 실행 이력 정리"""
        from datetime import datetime, timedelta
        ...
        deleted_count = (
            db.query(TaskExecutionHistory)
            .filter(TaskExecutionHistory.created_at < cleanup_date)
            .delete()
        )
        db.commit()
        return deleted_count

    def get_task_reliability_score(self, db: Session, *, task_id: str) -> dict:
        """작업 신뢰성 점수 계산"""
