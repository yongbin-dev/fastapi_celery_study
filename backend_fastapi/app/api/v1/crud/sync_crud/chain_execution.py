...
class CRUDChainExecution(CRUDBase[ChainExecution, dict, dict]):
    ...
    def get_by_status(
        ...
        return (
            ...
            .filter(ChainExecution.status == status.value)
            .order_by(desc(ChainExecution.created_at))
            .offset(skip)
            .limit(limit)
            .all()
    ...
    def get_running_chains(self, db: Session) -> List[ChainExecution]:
        ...
        return (
            db.query(ChainExecution)
            .filter(ChainExecution.status == ProcessStatus.STARTED.value)
    ...
    def get_failed_chains(
        ...
        return (
            db.query(ChainExecution)
            .filter(ChainExecution.status == ProcessStatus.FAILURE.value)
            .order_by(desc(ChainExecution.finished_at))
            .offset(skip)
            .limit(limit)
            .all()
    ...
    async def get_recent_chains(
        self, session: AsyncSession, *, days: int = 7, limit: int = 50
    ) -> List[ChainExecution]:
        """최근 N일간 체인 실행 목록 조회"""
