...
#
#     def update_status(
#         self, db: Session, *, chain_execution: ChainExecution, status: str
#     ) -> ChainExecution:
#         """체인 실행 상태 업데이트"""
#         chain_execution.status = status
#         if status == ProcessStatus.STARTED.value:
#             chain_execution.started_at = datetime.now()
#         elif status in [ProcessStatus.SUCCESS.value, ProcessStatus.FAILURE.value, ProcessStatus.REVOKED.value]:
...
#         return (
#             db.query(ChainExecution)
#             .filter(ChainExecution.created_at >= since_date)  # type: ignore
#             .order_by(desc(ChainExecution.created_at))  # type: ignore
#             .limit(limit)
#             .all()
#         )
#
#     def cleanup_old_chains(self, db: Session, *, days: int = 30) -> int:
#         """오래된 완료 체인 정리"""
#
...
#             .filter(
#                 and_(
#                     ChainExecution.finished_at < cleanup_date,  # type: ignore
#                     ChainExecution.status.in_(  # type: ignore
#                         [
#                             ProcessStatus.SUCCESS.value,
#                             ProcessStatus.FAILURE.value,
