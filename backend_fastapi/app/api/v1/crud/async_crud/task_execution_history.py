...
class AsyncCRUDTaskExecutionHistory(AsyncCRUDBase[TaskExecutionHistory, dict, dict]):
    ...
    async def get_by_task_id(
        ...
        *,
        ...
        try:
            ...
            ).order_by(desc(TaskExecutionHistory.attempt_number)).offset(skip).limit(limit)
    ...
    async def get_by_status(
        ...
        *,
        ...
        try:
            ...
            ).order_by(desc(TaskExecutionHistory.created_at)).offset(skip).limit(limit)
            result = await db.execute(stmt)
            return list(result.scalars().all())
        except Exception as e:
            await db.rollback()
    ...
    async def get_with_task(
        ...
        *,
        ...
        """작업 로그와 함께 실행 이력 조회"""
        try:
            stmt = select(TaskExecutionHistory).options(
                joinedload(TaskExecutionHistory.task)
            ).where(TaskExecutionHistory.task_id == task_id).order_by(
    ...
    async def get_recent_failures(
        ...
        *,
        ...
        try:
            ...
            ).order_by(desc(TaskExecutionHistory.created_at)).limit(limit)
            result = await db.execute(stmt)
            return list(result.scalars().all())
        except Exception as e:
            await db.rollback()
