...
class AsyncCRUDTaskMetadata(AsyncCRUDBase[TaskMetadata, dict, dict]):
    ...
    async def get_by_worker(
        self, db: AsyncSession, *, worker_name: str, skip: int = 0, limit: int = 100
    ) -> List[TaskMetadata]:
        """워커별 작업 메타데이터 목록 조회"""
        try:
            stmt = (
                select(TaskMetadata)
                .where(TaskMetadata.worker_name == worker_name)
    ...
    async def get_by_queue(
        self, db: AsyncSession, *, queue_name: str, skip: int = 0, limit: int = 100
    ) -> List[TaskMetadata]:
        """큐별 작업 메타데이터 목록 조회"""
        try:
            stmt = (
                select(TaskMetadata)
                .where(TaskMetadata.queue_name == queue_name)
                .order_by(desc(TaskMetadata.created_at))
                .offset(skip)
                .limit(limit)
            )
    ...
    async def get_scheduled_tasks(
        ...
        try:
            stmt = (
                ...
                .where(
                    and_(
                        TaskMetadata.eta.isnot(None), TaskMetadata.eta > datetime.now()
                    )
                )
    ...
    async def get_by_parent_id(
        self, db: AsyncSession, *, parent_id: str, skip: int = 0, limit: int = 100
    ) -> List[TaskMetadata]:
        """부모 작업 ID로 자식 작업들 조회"""
        try:
            stmt = (
                select(TaskMetadata)
                .where(TaskMetadata.parent_id == parent_id)
                .order_by(desc(TaskMetadata.created_at))
                .offset(skip)
                .limit(limit)
            )
    ...
    async def get_by_root_id(
        self, db: AsyncSession, *, root_id: str, skip: int = 0, limit: int = 100
    ) -> List[TaskMetadata]:
        """루트 작업 ID로 워크플로우 작업들 조회"""
        try:
            stmt = (
                select(TaskMetadata)
                .where(TaskMetadata.root_id == root_id)
                .order_by(desc(TaskMetadata.created_at))
                .offset(skip)
                .limit(limit)
            )
    ...
    async def get_worker_statistics(self, db: AsyncSession) -> List[dict]:
        ...
        try:
            stmt = (
                ...
                .where(TaskMetadata.worker_name.isnot(None))
    ...
    async def get_priority_distribution(self, db: AsyncSession) -> dict:
        ...
        try:
            stmt = (
                select(
                    TaskMetadata.priority, func.count(TaskMetadata.id).label("count")
                )
    ...
    async def get_workflow_tree(self, db: AsyncSession, *, root_id: str) -> dict:
        """워크플로우 트리 구조 조회"""
        try:
            # 루트 작업 조회
    ...
    async def get_recent_tasks_by_worker(
        self, db: AsyncSession, *, worker_name: str, days: int = 7, limit: int = 100
    ) -> List[TaskMetadata]:
