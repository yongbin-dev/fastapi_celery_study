...
async def create_ai_pipeline(
    ...
    return ResponseBuilder.success(data=result, message="AI 처리 파이프라인이 시작되었습니다")
...
@controller.get(
    "/ai-pipeline/{chain_id}/tasks", response_model=ApiResponse[PipelineStagesResponse]
)
async def get_pipeline_tasks(
    ...
    pipeline_stages = service.get_pipeline_tasks(
        redis_service=redis_service, chain_id=chain_id
    )
...
@controller.get(
    "/ai-pipeline/{chain_id}/stage/{stage}",
    response_model=ApiResponse[StageDetailResponse],
)
async def get_stage_task(
    ...
    stage_detail = service.get_stage_task(
        redis_service=redis_service, chain_id=chain_id, stage=stage
    )
