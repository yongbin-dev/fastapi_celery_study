...
class RedisPipelineStatusManager(AbstractPipelineStatusManager):
    ...
    def update_status(self, chain_id: str, stage: int, status: ProcessStatus, 
        ...
        try:
            ...
            logger.info(f"Pipeline {chain_id}: Stage {stage} - {status.value} ({progress}%)")
    ...
    def delete_pipeline(self, chain_id: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Pipeline {chain_id} 데이터 삭제 {'completed' if result else 'failed'}")
            return bool(result)
        except Exception as e:
...
class InMemoryPipelineStatusManager(AbstractPipelineStatusManager):
    """인메모리 기반 파이프라인 상태 관리 (테스트용)"""
    ...
    def update_status(self, chain_id: str, stage: int, status: ProcessStatus, 
                     progress: int = 0, metadata: Optional[Dict] = None, task_id: Optional[str] = None) -> bool:
        """인메모리에서 파이프라인 상태 업데이트"""
        try:
            if chain_id not in self._storage:
                self._storage[chain_id] = []
            
            pipeline_tasks = self._storage[chain_id]
            ...
            logger.error(f"인메모리 파이프라인 상태 업데이트 실패 (Chain: {chain_id}): {e}")
            return False

    def get_pipeline_status(self, chain_id: str) -> Optional[List[Dict]]:
        """파이프라인 전체 상태 조회"""
        return self._storage.get(chain_id)
    ...
    def delete_pipeline(self, chain_id: str) -> bool:
        """파이프라인 데이터 삭제"""
        try:
            if chain_id in self._storage:
                del self._storage[chain_id]
                return True
            return False
        except Exception as e:
            logger.error(f"인메모리 파이프라인 데이터 삭제 실패 (Chain: {chain_id}): {e}")
            return False
    ...
    def initialize_pipeline_stages(self, chain_id: str, input_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self._storage[chain_id] = [stage.to_dict() for stage in stages]
            return True
...
def get_status_manager(use_redis: bool = True) -> AbstractPipelineStatusManager:
    """
    상태 관리자 팩토리 함수
    
    Args:
        use_redis: Redis 사용 여부 (기본값: True, False시 InMemory 사용)
    
    Returns:
        AbstractPipelineStatusManager: 상태 관리자 인스턴스
    """
    if use_redis:
        return RedisPipelineStatusManager()
    else:
        return InMemoryPipelineStatusManager()